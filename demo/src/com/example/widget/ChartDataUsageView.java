...
import com.example.widget.ChartSweepView.OnSweepListener;
...
public class ChartDataUsageView extends ChartView {
	...
	public ChartDataUsageView(Context context, AttributeSet attrs, int defStyle) {
		...
		mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				// we keep dispatching repeating updates until
				// sweep is dropped
	...
	/**
	 * Update {@link #mVert} to both show data from
	 * {@link NetworkStatsHistory} and controls from {@link NetworkPolicy}.
	 */
	private void updateVertAxisBounds(ChartSweepView activeSweep) {
		...
		if (newMax != mVertMax) {
			...
			// since we just changed axis, make sweep recalculate
			// its value
			if (activeSweep != null) {
			...
			// TODO: find cleaner way of doing this, such as
			// requesting full
			// layout and making activeSweep discard its tracking
			// MotionEvent.
	...
	/**
	 * Control {@link ChartNetworkSeriesView#setEstimateVisible(boolean)}
	 * based on how close estimate comes to {@link #mSweepWarning}.
	 */
	...
	private void sendUpdateAxisDelayed(ChartSweepView sweep, boolean force) {
		if (force || !mHandler.hasMessages(MSG_UPDATE_AXIS, sweep)) {
			mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_UPDATE_AXIS, sweep), DELAY_MILLIS);
		}
	}
	...
	private OnSweepListener mVertListener = new OnSweepListener() {
		...
		public void onSweep(ChartSweepView sweep, boolean sweepDone) {
			if (sweepDone) {
				...
				// while moving, kick off delayed grow/shrink
				// axis updates
	 ...
	 * {@link ChartNetworkSeriesView} paints. Moves inspection ranges to be
	 * the last "week" of available data, without triggering listener
	 * events.
	 */
	...
	public static class DataAxis implements ChartAxis {
		...
		public float convertToPoint(long value) {
			if (LOG_SCALE) {
				// derived polynomial fit to make lower values
				// more visible
				final double normalized = ((double) value - mMin) / (mMax - mMin);
				final double fraction = Math.pow(10, 0.36884343106175121463 * Math.log10(normalized)
		...
		public long convertToValue(float point) {
			if (LOG_SCALE) {
				...
				final double fraction = 1.3102228476089056629 * Math.pow(normalized,
		...
		public long buildLabel(Resources res, SpannableStringBuilder builder, long value) {
			...
			final BytesResult result = Formatter.formatBytes(res, value, Formatter.FLAG_SHORTER
					| Formatter.FLAG_CALCULATE_ROUNDED);
		...
		public float[] getTickPoints() {
			...
			// target about 16 ticks on screen, rounded to nearest
			// power of 2
	...
	private static long roundUpToPowerOfTwo(long i) {
		...
		i--; // If input is a power of two, shift its high-order bit
			// right
