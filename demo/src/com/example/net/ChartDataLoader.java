...
import com.example.DataUsageSummary.AppItem;
...
public class ChartDataLoader extends AsyncTaskLoader<ChartData> {
	...
	public ChartData loadInBackground() {
		...
		try {
			...
			// since we can't do much without history, and we don't
			// want to
	...
	private ChartData loadInBackground(NetworkTemplate template, AppItem app, int fields) throws RemoteException {
		...
		if (app != null) {
			...
			for (int i = 0; i < size; i++) {
				...
				data.detailDefault = collectHistoryForUid(template, uid, SET_DEFAULT,
						data.detailDefault);
	...
	/**
	 * Collect {@link NetworkStatsHistory} for the requested UID, combining
	 * with an existing {@link NetworkStatsHistory} if provided.
	 */
