...
public class NetworkPolicyEditor {
	...
	private static NetworkPolicy buildDefaultPolicy(NetworkTemplate template) {
		// TODO: move this into framework to share with
		...
		return new NetworkPolicy(template, cycleDay, cycleTimezone, WARNING_DISABLED, LIMIT_DISABLED,
				SNOOZE_NEVER, SNOOZE_NEVER, metered, true);
	}
	...
	/**
	 * Build a revised {@link NetworkTemplate} that matches the same rule,
	 * but with an unquoted {@link NetworkTemplate#getNetworkId()}. Used to
	 * work around legacy bugs.
	 */
	private static NetworkTemplate buildUnquotedNetworkTemplate(NetworkTemplate template) {
		...
		if (!TextUtils.equals(strippedNetworkId, networkId)) {
			return new NetworkTemplate(template.getMatchRule(), template.getSubscriberId(),
					strippedNetworkId);
		} else {
			return null;
		}
