...
import com.example.UserAdapter.UserDetails;
import com.example.demo.R;
import com.example.drawable.CircleFramedDrawable;
...
public final class Utils {
	...
	/**
	 * Color spectrum to use to indicate badness. 0 is completely transparent
	 * (no data), 1 is most bad (red), the last value is least bad (green).
	 */
	public static final int[] BADNESS_COLORS = new int[] { 0x00000000, 0xffc43828, 0xffe54918, 0xfff47b00, 0xfffabf2c,
			0xff679e37, 0xff0a7f42 };
		...
		if (intent != null) {
			...
			for (int i = 0; i < listSize; i++) {
				...
				if ((resolveInfo.activityInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0) {
					...
					preference.setIntent(new Intent().setClassName(resolveInfo.activityInfo.packageName,
	...
	// updateTileToSpecificActivityFromMetaDataOrRemove(Context context,
	// DashboardTile tile) {
	//
	...
	/**
	 * Returns whether the device is voice-capable (meaning, it is also a
	 * phone).
	 */
	...
	public static Locale createLocaleFromString(String localeStr) {
		// TODO: is there a better way to actually construct a locale that will
		// match?
		// The main problem is, on top of Java specs, locale.toString() and
		// new Locale(locale.toString()).toString() do not return equal()
		// strings in
		// many cases, because the constructor takes the only string as the
		// language
	...
	public static void forcePrepareCustomPreferencesList(ViewGroup parent, View child, ListView list,
	...
	public static boolean copyMeProfilePhoto(Context context, UserInfo user) {
		...
		InputStream avatarDataStream = Contacts.openContactPhotoInputStream(context.getContentResolver(), contactUri,
				true);
	...
	private static String getLocalProfileGivenName(Context context) {
		...
		final Cursor localRawProfile = cr.query(Profile.CONTENT_RAW_CONTACTS_URI, new String[] { RawContacts._ID },
				RawContacts.ACCOUNT_TYPE + " IS NULL AND " + RawContacts.ACCOUNT_NAME + " IS NULL", null, null);
		...
		final Cursor structuredName = cr.query(
				Profile.CONTENT_URI.buildUpon().appendPath(Contacts.Data.CONTENT_DIRECTORY).build(), new String[] {
						CommonDataKinds.StructuredName.GIVEN_NAME, CommonDataKinds.StructuredName.FAMILY_NAME },
				Data.RAW_CONTACT_ID + "=" + localRowProfileId, null, null);
	 ...
	 * When launched in this mode, the given preference fragment will be
	 ...
	 * @param resultTo
	 *            Option fragment that should receive the result of the activity
	 *            launch.
	 * @param resultRequestCode
	 *            If resultTo is non-null, this is the request code in which to
	 *            report the result.
	 * @param titleResId
	 *            resource id for the String to display for the title of this
	 *            set of preferences.
	 * @param title
	 *            String to display for the title of this set of preferences.
	 */
	public static void startWithFragment(Context context, String fragmentName, Bundle args, Fragment resultTo,
			int resultRequestCode, int titleResId, CharSequence title) {
		startWithFragment(context, fragmentName, args, resultTo, resultRequestCode, null /* titleResPackageName */,
	 ...
	 *            If resultTo is non-null, this is the request code in which to
	 *            report the result.
	 * @param titleResPackageName
	 *            Optional package name for the resource id of the title.
	 * @param titleResId
	 *            resource id for the String to display for the title of this
	 *            set of preferences.
	 * @param title
	 ...
	 */
	public static void startWithFragment(Context context, String fragmentName, Bundle args, Fragment resultTo,
			int resultRequestCode, String titleResPackageName, int titleResId, CharSequence title) {
		startWithFragment(context, fragmentName, args, resultTo, resultRequestCode, titleResPackageName, titleResId,
				title, false /* not a shortcut */);
	}

	public static void startWithFragment(Context context, String fragmentName, Bundle args, Fragment resultTo,
			int resultRequestCode, int titleResId, CharSequence title, boolean isShortcut) {
		Intent intent = onBuildStartFragmentIntent(context, fragmentName, args, null /* titleResPackageName */,
	...
	public static void startWithFragment(Context context, String fragmentName, Bundle args, Fragment resultTo,
			int resultRequestCode, String titleResPackageName, int titleResId, CharSequence title, boolean isShortcut) {
		Intent intent = onBuildStartFragmentIntent(context, fragmentName, args, titleResPackageName, titleResId, title,
				isShortcut);
		if (resultTo == null) {
			context.startActivity(intent);
		} else {
	...
	public static void startWithFragmentAsUser(Context context, String fragmentName, Bundle args, int titleResId,
			CharSequence title, boolean isShortcut, UserHandle userHandle) {
		Intent intent = onBuildStartFragmentIntent(context, fragmentName, args, null /* titleResPackageName */,
				titleResId, title, isShortcut);
	...
	public static void startWithFragmentAsUser(Context context, String fragmentName, Bundle args,
			String titleResPackageName, int titleResId, CharSequence title, boolean isShortcut, UserHandle userHandle) {
		Intent intent = onBuildStartFragmentIntent(context, fragmentName, args, titleResPackageName, titleResId, title,
				isShortcut);
		intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
		intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK);
	 ...
	 * The implementation constructs an Intent that re-launches the current
	 ...
	 *            Optional package name for the resource id of the title.
	 * @param titleResId
	 *            Optional title resource id to show for this item.
	 * @param title
	 *            Optional title to show for this item.
	 * @param isShortcut
	 *            tell if this is a Launcher Shortcut or not
	 * @return Returns an Intent that can be launched to display the given
	 *         fragment.
	 */
	public static Intent onBuildStartFragmentIntent(Context context, String fragmentName, Bundle args,
		...
		// SubSettings.SavedAccessPointsSubSettings.class);
		// intent.putExtra(SettingsActivity.EXTRA_SHOW_FRAGMENT_AS_SUBSETTING,
		// true);
		// }else {
	...
	/**
	 * Creates a {@link UserAdapter} if there is more than one profile on the
	 * device.
	 * 
	 * <p>
	 * The adapter can be used to populate a spinner that switches between the
	 * Settings app on the different profiles.
	 * 
	 ...
	 * 
	 * The target user can be either the current user, the user that launched
	 * this activity or the user contained as an extra in the arguments or
	 * intent extras.
	 * 
	 * Note: This is secure in the sense that it only returns a target user
	 * different to the current one if the app launching this activity is the
	 * Settings app itself, running in the same user or in one that is in the
	 * same profile group, or if the user id is provided by the system.
	 */
	public static UserHandle getSecureTargetUser(IBinder activityToken, UserManager um, @Nullable Bundle arguments,
	 ...
	 * 
	 * You should use
	 * {@link #getSecureTargetUser(IBinder, UserManager, Bundle, Bundle)} if
	 * possible.
	 * 
	...
	public static Drawable getUserIcon(Context context, UserManager um, UserInfo user) {
		...
		return CircleFramedDrawable.getInstance(context,
				UserIcons.convertToBitmap(UserIcons.getDefaultUserIcon(user.id, /*
																				 * light
																				 * =
																				 */false)));
	}
	...
	/**
	 * Returns a label for the user, in the form of "User: user name" or
	 * "Work profile".
	 */
	...
	/**
	 * Return whether or not the user should have a SIM Cards option in
	 * Settings. TODO: Change back to returning true if count is greater than
	 * one after testing. TODO: See bug 16533525.
	 */
	...
	/**
	 * Determine whether a package is a "system package", in which case certain
	 * things (like disabling notifications or disabling the package altogether)
	...
	/**
	 * Queries for the UserInfo of a user. Returns null if the user doesn't
	 * exist (was removed).
	 * 
	 * @param userManager
	 *            Instance of UserManager
	 * @param checkUser
	 *            The user to check the existence of.
	...
	public static View inflateCategoryHeader(LayoutInflater inflater, ViewGroup parent) {
		final TypedArray a = inflater.getContext().obtainStyledAttributes(null,
				com.android.internal.R.styleable.Preference, com.android.internal.R.attr.preferenceCategoryStyle, 0);
	 ...
	 * 
	 * Note that for guest users, you should pass in
	 * {@code UserHandle.USER_NULL}.
	 * 
	 * @param userId
	 *            the user id or {@code UserHandle.USER_NULL} for a non-user
	 *            specific icon
	 */
		...
		if (filters != null && filters.size() > 0) {
			for (IntentFilter filter : filters) {
				if (filter.hasCategory(Intent.CATEGORY_BROWSABLE)
						&& (filter.hasDataScheme(IntentFilter.SCHEME_HTTP) || filter
								.hasDataScheme(IntentFilter.SCHEME_HTTPS))) {
	...
	// getLaunchByDeafaultSummary(ApplicationsState.AppEntry appEntry,
	...
	// int status = pm.getIntentVerificationStatus(packageName,
	// UserHandle.myUserId());
	// // consider a visible current link-handling state to be any explicitly
	// designated behavior
	...
	private static void setViewShown(final View view, boolean shown, boolean animate) {
		if (animate) {
			Animation animation = AnimationUtils.loadAnimation(view.getContext(), shown ? android.R.anim.fade_in
	...
	public static ApplicationInfo getAdminApplicationInfo(Context context, int profileId) {
		...
		try {
			...
			Log.e(TAG, "Error while retrieving application info for package " + mdmPackageName + ", userId "
					+ profileId, e);
			return null;
		}
	}
	...
	public static boolean isBandwidthControlEnabled() {
		final INetworkManagementService netManager = INetworkManagementService.Stub.asInterface(ServiceManager
	...
	public static SpannableString createAccessibleSequence(CharSequence displayText, String accessibileText) {
		...
		str.setSpan(new TtsSpan.TextBuilder(accessibileText).build(), 0, displayText.length(),
